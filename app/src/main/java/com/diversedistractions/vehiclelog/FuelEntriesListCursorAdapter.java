...
public class FuelEntriesListCursorAdapter extends CursorAdapter{
	private final LayoutInflater mInflater;
	
	public FuelEntriesListCursorAdapter(Context context, Cursor cursor) {
	...
	public View newView(Context context, Cursor cursor, ViewGroup parent) {
		return mInflater.inflate(R.layout.fuel_entries_list_row, parent, false);
	}
	
	@Override
	public void bindView(View view, Context context, Cursor cursor) {
		// Specify the columns we want to display in the list
		long date = cursor.getLong(cursor.getColumnIndex(DbFuelEntryTable.COL_FUEL_ENTRY_DATE));
		String odometer = cursor.getString(cursor.getColumnIndex
                (DbFuelEntryTable.COL_FUEL_ENTRY_ODOMETER));
		String gallons = cursor.getString(cursor.getColumnIndex
                (DbFuelEntryTable.COL_FUEL_ENTRY_GALLONS));
        //TODO: Should I add the cost to the list display?
		...
		((TextView) view.findViewById(R.id.gallons_text)).setText(gallons);
	}
